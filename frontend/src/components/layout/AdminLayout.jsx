...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-bg">
      <aside className="w-[230px] h-screen bg-surface border-r flex flex-col fixed left-0 top-0 bottom-0 z-40"
        ...
        <div className="px-5 py-6 border-b" style={{ borderColor: '#e0dcd4' }}>
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-display text-sm font-bold leading-tight" style={{ color: '#1a1a2e' }}>
                SquareEdgeSports
              </div>
              <div className="text-[9px] uppercase tracking-widest" style={{ color: '#6b6b7b' }}>Admin Panel</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 py-3 overflow-y-auto">
          {visibleNav.map((item, i) => item.section ? (
            <div key={i} className="text-[9px] uppercase tracking-widest font-bold px-5 pt-4 pb-2" style={{ color: '#9a9aaa' }}>{item.section}</div>
          ) : (
            <div key={item.path}
          ...
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl" style={{ background: '#f0ede8' }}>
            {photoSrc
              ...
              : <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                  style={{ background: '#1a1a2e' }}>{initials}</div>
            }
      ...
      <div className="ml-[230px] flex-1 flex flex-col h-screen overflow-hidden">
        <header className="flex-shrink-0 h-[58px] bg-surface border-b flex items-center justify-between px-7 z-30"
          ...
          <div className="font-display text-base font-bold" style={{ color: '#1a1a2e' }}>{currentLabel}</div>
